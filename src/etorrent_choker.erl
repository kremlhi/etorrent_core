...
-spec lookup_info(sets:set(), integer(), pid()) -> none | {seeding, float()}
                                                   | {leeching, float()}.
...
-spec build_rechoke_info(sets:set(), [pid()]) -> [#rechoke_info{}].
...
insert_new_peer_into_chain(Pid, [_|_] = Chain) ->
    Index = crypto:strong_rand_range(length(Chain) + 1),
